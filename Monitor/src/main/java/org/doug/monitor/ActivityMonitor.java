...
import org.doug.monitor.util.MathUtil;
import org.doug.monitor.util.Toaster;

public class ActivityMonitor extends Activity implements ActivityCompat.OnRequestPermissionsResultCallback {
...
//	private ScriptIntrinsicBlur intrinsic;
    ...
    private ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            mTVMemTotal.setText(MathUtil.mFormat.format(mSR.getMemTotal()) + Constans.kB);
            ...
            // When on ActivityProcesses the screen is rotated, ActivityMonitor is destroyed and back is pressed from ActivityProcesses
...
/*		} else {
        ...
        mLButtonRecord.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View arg0) {
                ...
                Toaster.showToast(ActivityMonitor.this, getString(id));
                return true;
            }
        });
        ...
        mLMenu.findViewById(R.id.LHelp).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                mPWMenu.dismiss();
                startActivity(new Intent(ActivityMonitor.this, ActivityHelp.class));
            }
        });
        ...
        mLMenu.findViewById(R.id.LAbout).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                mPWMenu.dismiss();
                startActivity(new Intent(ActivityMonitor.this, ActivityAbout.class));
            }
        });
...
//				mIVSettingsBG.getLayoutParams().height = settingsHeight;
        ...
        mLGraphicSurface.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View arg0) {
                Toaster.showToast(ActivityMonitor.this, getString(R.string.menu_settings_description));
                return true;
            }
        });
...
//				yBottom = (int) (getHeight()*0.88);
        ...
        mBChooseProcess.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent(ActivityMonitor.this, ActivityProcesses.class);
        ...
        mTVIntervalRead.setText(getString(R.string.interval_read) + " " + MathUtil.mFormatTime.format(intervalRead / (float) 1000) + " s");
        final TextView mTVIntervalUpdate = (TextView) findViewById(R.id.TVIntervalUpdate);
        mTVIntervalUpdate.setText(getString(R.string.interval_update) + " " + MathUtil.mFormatTime.format(intervalUpdate / (float) 1000) + " s");
        ...
        mSBRead.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                ...
                mTVIntervalRead.setText(getString(R.string.interval_read) + " " + MathUtil.mFormatTime.format(t / (float) 1000) + " s");
            }
        });
        ...
        mSBUpdate.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                ...
                mTVIntervalUpdate.setText(getString(R.string.interval_update) + " " + MathUtil.mFormatTime.format(t / (float) 1000) + " s");
            }
        });
        ...
        mCloseSettings.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (ActivityMonitor.this.intervalRead != intervalRead) {
                ...
                ActivityMonitor.this.intervalRead = intervalRead;
                ActivityMonitor.this.intervalUpdate = intervalUpdate;
                ActivityMonitor.this.intervalWidth = intervalWidth;
                ...
                (mLFeedback.findViewById(R.id.BFeedbackYes)).setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        try {
                            ...
                            e.printStackTrace();
                            startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(res.getString(R.string.github_site)))
                                    .addFlags(Intent.FLAG_ACTIVITY_NO_HISTORY | Intent.FLAG_ACTIVITY_CLEAR_WHEN_TASK_RESET | Intent.FLAG_ACTIVITY_MULTIPLE_TASK));
                        }
...
//		mLSettings.animate().setDuration(animTime).setInterpolator(new AccelerateDecelerateInterpolator()).y(sHeight + navigationBarHeight);
    ...
    private void setTextLabelCPU(TextView absolute, TextView percent, List<Float> values, @SuppressWarnings("unchecked") List<Integer>... valuesInteger) {
        if (valuesInteger.length == 1) {
            percent.setText(MathUtil.mFormatPercent.format(valuesInteger[0].get(0) * 100 / (float) mSR.getMemTotal()) + Constans.percent);
            mTVMemoryAM.setVisibility(View.VISIBLE);
            mTVMemoryAM.setText(MathUtil.mFormat.format(valuesInteger[0].get(0)) + Constans.kB);
        } else if (!values.isEmpty()) {
            percent.setText(MathUtil.mFormatPercent.format(values.get(0)) + Constans.percent);
    ...
    private void setTextLabelMemory(TextView absolute, TextView percent, List<String> values) {
        if (!values.isEmpty()) {
            absolute.setText(MathUtil.mFormat.format(Integer.parseInt(values.get(0))) + Constans.kB);
            percent.setText(MathUtil.mFormatPercent.format(Integer.parseInt(values.get(0)) * 100 / (float) mSR.getMemTotal()) + Constans.percent);
        }
    }
    ...
    private void setTextLabelCPUProcess(LinearLayout l) {
        ...
        if (entry != null
            ...
            if (processesMode == Constans.processesModeShowCPU)
                ((TextView) l.findViewById(R.id.TVpPercentage)).setText(MathUtil.mFormatPercent.format(((List<String>) entry.get(Constans.pFinalValue)).get(0)) + Constans.percent);
            else
                ((TextView) l.findViewById(R.id.TVpPercentage)).setText(MathUtil.mFormatPercent.format(((List<Integer>) entry.get(Constans.pTPD)).get(0) * 100 / (float) mSR.getMemTotal()) + Constans.percent);
    }
    ...
    private void setTextLabelMemoryProcesses(LinearLayout l) {
        ...
        if (processesMode == Constans.processesModeShowCPU)
            ...
            if (entry != null
                ...
                tv.setText(MathUtil.mFormat.format(((List<String>) entry.get(Constans.pTPD)).get(0)) + Constans.kB);
            }
        }
    }
...
//					if (true) {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == Constans.storagePermission && PackageManager.PERMISSION_DENIED == grantResults[0]) {
            Toaster.showToast(ActivityMonitor.this, getString(R.string.w_main_storage_permission));
        }
    }
}
