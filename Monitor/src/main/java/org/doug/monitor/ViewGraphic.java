            ...
            for (int n = 0; n < l.size(); ++n) {
                if ((Boolean) l.get(n).get(Constans.pSelected)) {
                    ...
                    if (processesMode == Constans.processesModeShowCPU) // processesMode==0 CPU usage, processesMode==1 Memory
                        ...
                        drawLineInteger((List<Integer>) l.get(n).get(Constans.pTPD), canvas, paint);
                }
            }
...
//		canvas.drawText("20%", tempVar, yTop + graphicHeight*0.8f + yLegendSpace, textPaintLegendV);
        ...
        if (mThread.isInterrupted())
            return;
        canvas.drawText(readIntervalText + " " + MathUtil.mFormatTime.format(mSR.getIntervalRead() / (float) 1000) + " s", xLeft + 15, yTop + 5 + yTopSeparation, textPaintInside);
        if (mThread.isInterrupted())
            return;
        canvas.drawText(updateIntervalText + " " + MathUtil.mFormatPercent.format(mSR.getIntervalUpdate() / (float) 1000) + " s", xLeft + 15, yTop + 5 + yTopSeparation * 2, textPaintInside);
        if (mThread.isInterrupted())
            return;
